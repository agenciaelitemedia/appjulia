        ...
        <div className="p-4 space-y-5">
          ...
          {conversationHistory.length > 0 && (
            <>
              <Separator />
              <div className="space-y-3">
                <h5 className="text-xs font-semibold uppercase text-muted-foreground">Eventos</h5>
                <div className="space-y-2">
                  {conversationHistory.slice(0, 10).map(entry => (
                    <div key={entry.id} className="text-xs text-muted-foreground">
                      <span className="font-medium text-foreground">{entry.actor_name}</span>
                      {' '}{entry.action === 'opened' ? 'abriu a conversa' :
                            entry.action === 'closed' ? 'encerrou a conversa' :
                            entry.action === 'resolved' ? 'resolveu a conversa' :
                            entry.action === 'assigned' ? `transferiu para ${entry.to_value}` :
                            entry.action === 'reopened' ? 'reabriu a conversa' :
                            entry.action}
                      {' — '}
                      <span>{format(new Date(entry.created_at), 'dd/MM HH:mm', { locale: ptBR })}</span>
                      {entry.notes && <p className="mt-0.5 italic">{entry.notes}</p>}
                    </div>
                  ))}
                </div>
              </div>
