...
import { X, Phone, MessageSquare, Clock, Tag, History, Plus, Hash, Check, Pencil, Info, FileText, Search, Users, UserCheck, Layers, Megaphone } from 'lucide-react';
...
const actionLabels: Record<string, (e: ConversationHistoryEntry) => string> = {
  ...
  priority_changed: (e) => `mudou a prioridade${e.from_value ? ` de ${e.from_value}` : ''} para ${e.to_value}`,
  queue_changed:    (e) => `mudou para a fila ${e.to_value}`,
  transferred:      (e) => `transferiu para ${e.to_value}`,
  status_changed:   (e) => `mudou o status para ${e.to_value}`,
  snoozed:          (e) => `adiou a conversa até ${e.to_value}`,
  tag_added:        (e) => `adicionou a etiqueta "${e.to_value}"`,
  tag_removed:      (e) => `removeu a etiqueta "${e.to_value}"`,
  note_added:       () => 'adicionou uma nota interna',
  returned_to_queue: () => 'devolveu para a fila de atendimento',
  auto_returned:    () => 'devolveu para a fila automaticamente (tempo esgotado)',
