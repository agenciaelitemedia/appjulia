...
import { MvpChatDetailsPanel } from './MvpChatDetailsPanel';
 ...
 */
export function MvpChatRightBar({ row, isBelowLg }: { row: MvpChatRowData | null; isBelowLg: boolean }) {
  const { selectedContact, showDetailPanel, setShowDetailPanel } = useWhatsAppData();

  if (!selectedContact) {
    return <MvpChatDetailsPanel row={row} />;
  }
