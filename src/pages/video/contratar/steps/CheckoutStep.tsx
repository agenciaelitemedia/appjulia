...
export function CheckoutStep({ orderId, checkoutUrl, onProvisioned }: Props) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`video-order-${orderId}`)
      .on('postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'video_orders', filter: `id=eq.${orderId}` },
      ...
      <CardContent className="space-y-4">
        {status === 'pending' && (
          <>
            <div className="text-sm text-muted-foreground">
              Você será redirecionado para o Mercado Pago. Após o pagamento, voltaremos para liberar seu acesso automaticamente.
            </div>
        ...
        {status === 'paid' && (
          <div className="text-center py-6 space-y-2">
            ...
            <div className="text-sm text-muted-foreground flex items-center gap-2 justify-center">
              <Loader2 className="h-4 w-4 animate-spin" /> Liberando seu plano de videochamadas…
            </div>
            {error && (
        ...
        {status === 'provisioned' && (
          <div className="text-center py-6">
            ...
            <div className="font-semibold">Acesso liberado!</div>
          </div>
        )}
        ...
        {status === 'failed' && (
          <div className="text-center py-6 space-y-2">
            ...
            <div className="text-sm text-muted-foreground">Tente novamente ou entre em contato com nosso suporte.</div>
          </div>
        )}
      </CardContent>
    </Card>
