...
export default function ContratarFilasPage() {
  const { user } = useAuth();
  ...
  async function handleCreateOrder() {
    if (!user?.id || !draft.plan) { toast.error('Usuário não autenticado'); return; }
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('queue-order-create', {
        body: {
          client_id: clientId,
          plan_id: draft.plan.id,
          billing_period: draft.billing_period,
          extra_queues: draft.extra_queues,
      ...
      if (error || !data?.order_id) throw new Error(error?.message || data?.error || 'Falha ao criar pedido');
      const oid = data.order_id;
      setOrderId(oid);

      if (data?.price_validation && data.price_validation.ok === false) {
        toast.warning('Atenção: o valor exibido difere do valor cobrado. Confira o total no pagamento.');
      }

      const { data: ck, error: ckErr } = await supabase.functions.invoke('queue-order-checkout', {
        body: { order_id: oid },
      });
      if (ckErr || !ck?.checkout_url) throw new Error(ckErr?.message || ck?.error || 'Falha ao gerar pagamento');
    ...
    <div className="container mx-auto py-6 max-w-6xl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <MessageSquare className="h-6 w-6" /> Contratar Filas
        </h1>
        <p className="text-sm text-muted-foreground">
          Escolha um plano de filas, confirme seus dados e ative em segundos.
        </p>
      </div>

      <Stepper step={step} />

      {step === 'plan' && <SelectPlanStep draft={draft} onChange={setDraft} onNext={() => setStep('data')} />}
      {step === 'data' && <ConfirmDataStep draft={draft} onChange={setDraft} onNext={handleCreateOrder} onBack={() => setStep('plan')} busy={busy} />}
      ...
      {step === 'success' && (
        <Card className="max-w-2xl mx-auto">
          <CardContent className="text-center py-12 space-y-4">
            ...
            <p className="text-muted-foreground">Seu plano de filas está ativo. Você já pode criar e gerenciar suas filas.</p>
            <div className="flex gap-3 justify-center pt-2">
              <Button variant="outline" onClick={() => navigate('/dashboard')}>Ir para painel</Button>
              <Button onClick={() => navigate('/agente/filas')}>Acessar filas</Button>
            </div>
          </CardContent>
        </Card>
      )}
