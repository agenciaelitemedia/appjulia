...
import { useQueuePlansAdmin, type QueuePlan } from '../hooks/useQueuePlansAdmin';
import { QueuePlanDialog } from './QueuePlanDialog';

export function QueuePlansTab() {
  const { plans, plansLoading, createPlan, updatePlan, deletePlan } = useQueuePlansAdmin();
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingPlan, setEditingPlan] = useState<QueuePlan | null>(null);

  const handleSave = (plan: Partial<QueuePlan>) => {
    if (editingPlan) {
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle className="text-lg">Planos de Filas</CardTitle>
              ...
              <TableBody>
                {plans.map((plan) => (
                  <TableRow key={plan.id}>
                    ...
                    <TableCell>{plan.max_queues}</TableCell>
                    ...
                    <TableCell className="text-xs">R$ {Number(plan.extra_queue_price).toFixed(2)}</TableCell>
                    <TableCell>
