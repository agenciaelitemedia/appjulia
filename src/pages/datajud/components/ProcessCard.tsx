import { Calendar, Building2, Scale, ArrowRight, ChevronRight } from 'lucide-react';
...
export function ProcessCard({ hit, tribunalKey, onViewDetails }: ProcessCardProps) {
  const process = hit._source;
  const lastMovement = process.movimentos?.[0];
  
  // Get tribunal category for color
    ...
    >
      <CardContent className="p-5">
        <div className="flex flex-col gap-4">
          {/* Header with process number and tribunal */}
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <Badge
                  ...
                  className={cn(
                    'text-xs font-medium',
                    getTribunalColor(getTribunalCategory(tribunalKey))
                  )}
                >
                  {tribunalKey}
                </Badge>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    {formatDate(lastMovement.dataHora)}
                  </p>
                </div>
              </div>
            </div>
