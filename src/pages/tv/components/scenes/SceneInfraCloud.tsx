...
import { useInfraStats, useWebhookActivity, useMediaStats } from '../../hooks/useInfraStats';
import { useDispatcherHealth } from '@/pages/configuracoes/hooks/useUazapiHistoryRuns';
import { Database, Cloud, Webhook, Image as ImageIcon, Activity } from 'lucide-react';
  ...
  return (
    <div className="grid grid-cols-12 gap-6 h-full">
      ...
      <TvCard
        ...
        rightSlot={<Webhook className="h-5 w-5 text-zinc-500" />}
      >
        <div className="grid grid-cols-3 gap-3 mb-4">
          <div className="rounded-lg bg-zinc-900 p-3 text-center">
            ...
            <div className="text-3xl font-bold tabular-nums text-zinc-100">
              {(webhooks?.total_24h ?? 0).toLocaleString('pt-BR')}
            </div>
          </div>
          <div className="rounded-lg bg-zinc-900 p-3 text-center">
            ...
            <div className="text-3xl font-bold tabular-nums text-emerald-300">
              {(webhooks?.total_1h ?? 0).toLocaleString('pt-BR')}
            </div>
          </div>
          <div className="rounded-lg bg-zinc-900 p-3 text-center">
            ...
            <div className="text-3xl font-bold tabular-nums text-violet-300">
              {(webhooks?.forwarded_24h ?? 0).toLocaleString('pt-BR')}
            </div>
          </div>
        </div>
        <BarRanking
          items={(webhooks?.per_source ?? []).map((p) => ({
            id: p.source,
            label: p.source,
            value: p.count,
          }))}
