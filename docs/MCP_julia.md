...
| Tool | Argumentos | Retorno | Origem |
| --- | --- | --- | --- |
| `julia_chat_localizar` (v1.0.0) | `identificador` (telefone com/sem máscara, com/sem DDI 55, com/sem 9º dígito · protocolo `#AAAA-NNNNNN` · `conversation_id`), `timezone` | busca direta e conclusiva: varre TODOS os status e também atendimentos pausados, sem janela de tempo nem paginação. Devolve `itens[]` com contato, telefone, protocolo, fila, responsável, `assigned_at`, snooze, ticket e `conversation_id`. Vazio significa que o identificador **não pertence ao escritório do token** (não paginar) | `chat_contacts`, `chat_conversations`, `queues` |
| `julia_chat_listar_conversas` (v1.3.0) | `status`, `tab`, `queue_id(s)`, `responsavel`, `assigned_user_id`, `unassigned`, `busca` (nome, telefone ou protocolo), **`incluir_pausados`**, `periodo`, **`data`** (dia civil YYYY-MM-DD), **`de`/`ate`**, **`timezone`**, `tag_ids`, `prioridade`, `com_ticket`, `com_crm_builder`, `sla`, `ordenar`, `incluir_origem_campanha`, `limite`, `offset` | mesma consulta unificada da tela `/chat` (RPC `chat_list_feed`), em envelope `{json,text}` com `itens[]` e bloco **`origem`** por lead (canal, `canal_origem`, fila, `cod_agent`, campanha de anúncio do legado `campaing_ads` e frase-resumo) + fila, protocolo, não lidas, última mensagem, SLA, etiquetas, ticket, CRM Builder, etapa CRM Julia, snooze, contadores, `janela`, `busca` (tipo detectado) e `proximo_offset`. **Busca por telefone/protocolo é identificador**: o telefone é normalizado para os 8 últimos dígitos (cobre 12 e 13 dígitos), os pausados entram automaticamente e resultado vazio é conclusivo (`pagination.has_more=false`, sem sugestão de paginação). **Janelas de tempo são resolvidas no fuso informado (padrão `America/Sao_Paulo`)** | RPC `chat_list_feed` + banco legado (`campaing_ads`, via `db-query`) |
