...
| Tool | Argumentos | Retorno | Origem |
| --- | --- | --- | --- |
| `julia_chat_listar_conversas` (v1.2.0) | `status`, `tab`, `queue_id(s)`, `responsavel`, `assigned_user_id`, `unassigned`, `busca`, `periodo`, **`data`** (dia civil YYYY-MM-DD), **`de`/`ate`**, **`timezone`**, `tag_ids`, `prioridade`, `com_ticket`, `com_crm_builder`, `sla`, `ordenar`, `incluir_origem_campanha`, `limite`, `offset` | mesma consulta unificada da tela `/chat` (RPC `chat_list_feed`), agora em envelope `{json,text}` com `itens[]` e bloco **`origem`** por lead (canal, `canal_origem`, fila, `cod_agent`, campanha de anúncio do legado `campaing_ads` e frase-resumo) + fila, protocolo, não lidas, última mensagem, SLA, etiquetas, ticket, CRM Builder, etapa CRM Julia, snooze, contadores, `janela` e `proximo_offset`. **Janelas de tempo são resolvidas no fuso informado (padrão `America/Sao_Paulo`)**: `data` cobre 00:00:00.000–23:59:59.999 locais; o JSON devolve os limites convertidos para UTC em `janela.de/ate` e `coverage.from/to` | RPC `chat_list_feed` + banco legado (`campaing_ads`, via `db-query`) |
