...
// Gera URL de pagamento Mercado Pago para um telephony_order.
// =====================================================
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const items = [{
      title: `Telefonia ${order.plan_name} (${PERIOD_LABELS[order.billing_period]})`,
      quantity: 1,
    ...
    const preference = {
      ...
      back_urls: {
        success: `${siteUrl}/telefonia/contratar/sucesso`,
        failure: `${siteUrl}/telefonia/contratar?status=failure`,
        pending: `${siteUrl}/telefonia/contratar?status=pending`,
      },
      auto_return: 'approved',
      statement_descriptor: 'JULIA-TEL',
      metadata: { order_type: 'telephony', order_id },
    }
    ...
    if (!resp.ok) {
      return json({ error: `Mercado Pago erro ${resp.status}: ${respText}` }, 400)
    }
    ...
    const orderNsu = `TEL-MP-${Date.now()}-${Math.random().toString(36).substring(2, 8).toUpperCase()}`
    ...
    console.error('[telephony-order-checkout] Error:', err)
