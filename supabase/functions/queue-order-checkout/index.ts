...
// Gera URL de pagamento Mercado Pago para um queue_order.
// =====================================================
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const items = [{
      title: `Filas ${order.plan_name} (${PERIOD_LABELS[order.billing_period]})`,
      quantity: 1,
    ...
    const preference = {
      ...
      back_urls: {
        success: `${siteUrl}/filas/contratar?status=success`,
        failure: `${siteUrl}/filas/contratar?status=failure`,
        pending: `${siteUrl}/filas/contratar?status=pending`,
      },
      auto_return: 'approved',
      statement_descriptor: 'JULIA-FIL',
      metadata: { order_type: 'queue', order_id },
    }
    ...
    const orderNsu = `FIL-MP-${Date.now()}-${Math.random().toString(36).substring(2, 8).toUpperCase()}`
    ...
    console.error('[queue-order-checkout] Error:', err)
