-- Schedule the internal-notification-scheduler edge function to run every minute.
DO $$
BEGIN
  PERFORM cron.unschedule('internal-notification-scheduler-every-minute');
EXCEPTION WHEN OTHERS THEN NULL;
END $$;

SELECT cron.schedule(
  'internal-notification-scheduler-every-minute',
  '* * * * *',
  $cron$
  SELECT net.http_post(
    url := 'https://zenizgyrwlonmufxnjqt.supabase.co/functions/v1/internal-notification-scheduler',
