CREATE INDEX IF NOT EXISTS idx_crm_deals_link_conversation
  ON public.crm_deals (((custom_fields->'links'->'chat'->>'conversation_id')))
  WHERE (custom_fields->'links'->'chat'->>'conversation_id') IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_crm_deals_link_contact
  ON public.crm_deals (((custom_fields->'links'->'chat'->>'contact_id')))
  WHERE (custom_fields->'links'->'chat'->>'contact_id') IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_chat_conversation_tags_conv
...
BEGIN
  ...
  v_sql := format($q$
    ...
    SELECT jsonb_build_object(
      ...
      'rows', COALESCE((SELECT jsonb_agg(to_jsonb(p) - 'sla_first_target' - 'sla_nrt_target' - 'sla_res_target') FROM page p), '[]'::jsonb)
    )
