CREATE OR REPLACE FUNCTION public.chat_list_feed(p_client_id text, p_queue_ids uuid[] DEFAULT NULL::uuid[], p_status text DEFAULT NULL::text, p_tab text DEFAULT NULL::text, p_owner text DEFAULT NULL::text, p_unassigned boolean DEFAULT NULL::boolean, p_search text DEFAULT NULL::text, p_from timestamp with time zone DEFAULT NULL::timestamp with time zone, p_to timestamp with time zone DEFAULT NULL::timestamp with time zone, p_tag_ids uuid[] DEFAULT NULL::uuid[], p_priority text DEFAULT NULL::text, p_has_ticket boolean DEFAULT NULL::boolean, p_has_crm_builder boolean DEFAULT NULL::boolean, p_sort text DEFAULT 'recent'::text, p_limit integer DEFAULT 30, p_offset integer DEFAULT 0, p_owners text[] DEFAULT NULL::text[], p_sla_status text[] DEFAULT NULL::text[], p_hide_snoozed boolean DEFAULT true, p_restrict_open_to text[] DEFAULT NULL::text[])
 RETURNS jsonb
 LANGUAGE plpgsql
...
DECLARE
  ...
  v_owners_norm text[];
  v_restrict_norm text[];
BEGIN
  IF p_owners IS NOT NULL AND array_length(p_owners, 1) > 0 THEN
    SELECT array_agg(DISTINCT btrim(lower(x))) INTO v_owners_norm
      FROM unnest(p_owners) x WHERE btrim(COALESCE(x,'')) <> '';
  END IF;
  IF p_restrict_open_to IS NOT NULL AND array_length(p_restrict_open_to, 1) > 0 THEN
    SELECT array_agg(DISTINCT btrim(lower(x))) INTO v_restrict_norm
      FROM unnest(p_restrict_open_to) x WHERE btrim(COALESCE(x,'')) <> '';
  END IF;
  ...
  IF v_owners_norm IS NOT NULL AND array_length(v_owners_norm, 1) > 0 THEN
    IF p_unassigned IS TRUE THEN
      v_where := v_where || format(
        ' AND (btrim(lower(COALESCE(b.assigned_to,''''))) = ANY(%1$L::text[]) OR b.assigned_user_id::text = ANY(%1$L::text[]) OR COALESCE(btrim(b.assigned_to), '''') = '''')',
        v_owners_norm);
    ELSE
      v_where := v_where || format(
        ' AND (btrim(lower(COALESCE(b.assigned_to,''''))) = ANY(%1$L::text[]) OR b.assigned_user_id::text = ANY(%1$L::text[]))',
        v_owners_norm);
    END IF;
  ELSIF p_owner IS NOT NULL AND btrim(p_owner) <> '' THEN
    IF p_unassigned IS TRUE THEN
      v_where := v_where || format(
        ' AND (btrim(lower(COALESCE(b.assigned_to,''''))) = %1$L OR b.assigned_user_id::text = %1$L OR COALESCE(btrim(b.assigned_to), '''') = '''')',
        btrim(lower(p_owner)));
    ELSE
      v_where := v_where || format(
        ' AND (btrim(lower(COALESCE(b.assigned_to,''''))) = %1$L OR b.assigned_user_id::text = %1$L)',
        btrim(lower(p_owner)));
    END IF;
  ELSIF p_unassigned IS TRUE THEN
    v_where := v_where || ' AND COALESCE(btrim(b.assigned_to), '''') = ''''';
  ...
  END IF;
  IF v_restrict_norm IS NOT NULL AND array_length(v_restrict_norm, 1) > 0 THEN
    v_where := v_where || format(
      ' AND (b.status <> ''open'' OR btrim(lower(COALESCE(b.assigned_to,''''))) = ANY(%1$L::text[]) OR b.assigned_user_id::text = ANY(%1$L::text[]))',
      v_restrict_norm);
  END IF;
  ...
  v_sql := format($q$
    ...
    page_ids AS (
      SELECT l.*,
        ...
        , row_number() OVER (ORDER BY %3$s) AS ord
    ...
    SELECT jsonb_build_object(
      ...
      'rows', COALESCE((SELECT jsonb_agg(to_jsonb(p) - 'sla_first_target' - 'sla_nrt_target' - 'sla_res_target' - 'rn' - 'ord' ORDER BY p.ord) FROM page p), '[]'::jsonb)
    )
