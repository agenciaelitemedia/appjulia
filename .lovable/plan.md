...
- **Regra geral de imports:** qualquer outro lugar do sistema que hoje usa componentes/hooks do chat (CRM, CRM Builder, Contatos, Tickets, Escritórios, Telefonia, Notificações, painéis admin, X-Julia, etc.) passa a importar as cópias do JulIA Chat — nunca mais os arquivos legados. Exemplos já mapeados: `ChatSidePanel`, `ChatCrmButton`, `ChatLinkedDealSheet`, `ChatTicketSidePanel`, `ChatTicketDetailSidePanel`, `CreateCrmCardSheet`, `WavoipCallButton`, `SlaBadge`, `PriorityBadge`, `GenerateSummaryButton`, `SummaryCard`, `MediaLightbox`, `NewConversationDialog`, `lib/chat/*`.
- Levantamento antes de editar: rodar a busca por importadores de `@/components/chat`, `@/contexts/WhatsAppDataContext`, `@/lib/chat` e hooks `useChat*` fora de `src/pages/chat/ChatPage.tsx`, e repontar todos para o módulo.
- Único consumidor autorizado dos arquivos legados: a árvore do `/chat-old` (`src/pages/chat/ChatPage.tsx` → `ChatContainer` e o que ele importa). Nada fora dela pode referenciar a pasta antiga; ao descontinuar o `/chat-old`, a exclusão dos arquivos legados não deve quebrar nenhuma outra tela.
